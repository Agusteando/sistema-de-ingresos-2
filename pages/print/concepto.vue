    ...
    <div class="max-w-[1180px] mx-auto border border-gray-200 p-8 rounded-2xl print:border-none print:p-4 relative bg-white min-h-[900px] flex flex-col">
      <div class="flex justify-between items-start mb-6 border-b border-gray-200 pb-5">
        ...
        <div class="text-center flex-1 mx-4">
          ...
          <div class="mt-2 text-[12px] font-semibold text-gray-700">{{ reportTitle }}</div>
          <div class="text-[12px] text-gray-600">{{ conceptLabel }}</div>
          <div class="text-[11px] text-gray-500">{{ reportContextLabel }}</div>
        </div>
      ...
      <div v-if="isMovementsMode" class="grid grid-cols-4 gap-3 mb-6 text-[11px]">
      ...
      <div v-else-if="isDebtorsMode" class="grid grid-cols-4 gap-3 mb-6 text-[11px]">
        <div class="border border-gray-200 rounded-lg p-3">
          <div class="text-gray-500 uppercase font-bold">Deudores</div>
          <div class="text-gray-900 font-bold text-[14px]">{{ resumen.alumnos || 0 }}</div>
        </div>
        <div class="border border-gray-200 rounded-lg p-3">
          <div class="text-gray-500 uppercase font-bold">Saldo pendiente</div>
          <div class="text-gray-900 font-bold text-[14px]">${{ Number(resumen.saldoPendiente || 0).toFixed(2) }}</div>
        </div>
        <div class="border border-gray-200 rounded-lg p-3">
          <div class="text-gray-500 uppercase font-bold">Cargos exigibles</div>
          <div class="text-gray-900 font-bold text-[14px]">${{ Number(resumen.totalCargos || 0).toFixed(2) }}</div>
        </div>
        <div class="border border-gray-200 rounded-lg p-3">
          <div class="text-gray-500 uppercase font-bold">Umbral</div>
          <div class="text-gray-900 font-bold text-[14px]">&gt; ${{ Number(resumen.threshold || 0).toFixed(2) }}</div>
        </div>
      </div>
      ...
      <div v-if="isMovementsMode" class="mb-4 text-[10px] text-gray-600">
      ...
      <div v-else-if="isDebtorsMode" class="mb-4 text-[10px] text-gray-600">
        Incluye únicamente alumnos con saldo exigible mayor al umbral en los conceptos seleccionados. El cálculo usa la misma lógica de la cartera Deudores de Aurora, incluidos sus pagos registrados, conciliaciones pendientes y fechas límite especiales.
      </div>
      ...
      <table v-if="isMovementsMode" class="w-full text-[8px] border-collapse mb-8">
        <thead>
      ...
      <table v-else-if="isDebtorsMode" class="w-full text-[8px] border-collapse mb-8">
        <thead>
          <tr class="border-b border-gray-300">
            <th class="py-2 text-left font-semibold text-gray-600 uppercase">Matrícula</th>
            ...
            <th class="py-2 text-right font-semibold text-gray-600 uppercase">Pagado</th>
            <th class="py-2 text-right font-semibold text-gray-600 uppercase">Saldo</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="!rows.length">
          ...
          <tr v-else v-for="r in rows" :key="r.matricula" class="border-b border-gray-100">
            <td class="py-2 text-gray-900 font-mono">{{ r.matricula }}</td>
            <td class="py-2 text-gray-900 font-semibold">{{ r.nombreCompleto || '—' }}</td>
            <td class="py-2 text-gray-900">{{ r.nivel || '—' }}</td>
            <td class="py-2 text-gray-900">{{ r.grado || '—' }}</td>
            ...
            <td class="py-2 text-right text-gray-900">${{ Number(r.totalPagado || 0).toFixed(2) }}</td>
            <td class="py-2 text-right font-bold text-gray-900">${{ Number(r.saldoPendiente || 0).toFixed(2) }}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div v-if="isMovementsMode" class="mt-auto border-t border-gray-300 pt-5 flex justify-end">
      ...
      <div v-else-if="isDebtorsMode" class="mt-auto border-t border-gray-300 pt-4 text-[10px] text-gray-600 flex justify-between gap-6">
        <span>Plantel: {{ reportPlantel || '—' }} · {{ cycleLabel }}</span>
        <span>{{ resumen.alumnos || 0 }} deudores · Saldo: ${{ Number(resumen.saldoPendiente || 0).toFixed(2) }}</span>
      </div>
...
const rows = ref([])
const conceptos = ref([])
const resumen = ref({ total: 0, totalRegistrado: 0, totalNoAplicado: 0, transacciones: 0, alumnos: 0, formasPago: [], inscritos: 0, completos: 0, cobertura: 0, conceptosFaltantes: 0, asignacionesFaltantes: 0, saldoPendiente: 0, totalCargos: 0, totalPagado: 0, threshold: 0 })
...
const isMovementsMode = computed(() => reportMode.value === 'movements')
const isMissingMode = computed(() => reportMode.value === 'missing')
const isDebtorsMode = computed(() => reportMode.value === 'debtors')
const cycleLabel = computed(() => reportCycleLabel.value || String(route.query.ciclo || 'Ciclo escolar'))
const reportTitle = computed(() => isMissingMode.value
  ? 'Alumnos inscritos sin concepto'
  : (isDebtorsMode.value ? 'Deudores por concepto' : 'Reporte por concepto'))
const reportContextLabel = computed(() => isMissingMode.value
  ? `${cycleLabel.value} · Solo alumnos inscritos`
  : (isDebtorsMode.value ? `${cycleLabel.value} · Saldo exigible` : 'Histórico financiero del periodo seleccionado'))
...
const formatDebtorConcepts = (items) => (Array.isArray(items) ? items : [])
  .map(item => `${item.concepto || 'Concepto'} ($${Number(item.saldo || 0).toFixed(2)})`)
  .join(', ') || '—'
...
onMounted(async () => {
  ...
  try {
    ...
    reportMode.value = ['missing', 'debtors'].includes(res.modo) ? res.modo : 'movements'

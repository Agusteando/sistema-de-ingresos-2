  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-container large">
      <div class="modal-header">
        <h2 style="font-size: 1.25rem; font-weight: 700; color: var(--brand-campus);">
          {{ isEdit ? 'Edición de Expediente' : 'Apertura de Nuevo Ingreso' }}
        </h2>
      </div>
        ...
        <div class="modal-content">
          <div class="grid-3">
            <div class="form-group">
              ...
              <input type="text" v-model="form.matricula" class="input-field" required :disabled="isEdit">
            </div>
            <div class="form-group" style="grid-column: span 2;">
              <label class="form-label">Nombre Completo de Alumno</label>
              <input type="text" v-model="form.nombreCompleto" class="input-field" required>
            </div>
            ...
            <div class="form-group"><label class="form-label">Género</label>
              <select v-model="form.genero" class="input-field" required><option value="1">Masculino</option><option value="0">Femenino</option></select>
            </div>
            <div class="form-group"><label class="form-label">Plantel</label>
              <select v-model="form.plantel" class="input-field" required>
                <option value="PT">Primaria Toluca</option><option value="PM">Primaria Metepec</option><option value="SM">Secundaria Metepec</option>
              </select>
            </div>

            <div class="form-group"><label class="form-label">Nivel Educativo</label>
              <select v-model="form.nivel" class="input-field" required>
                <option value="Preescolar">Preescolar</option><option value="Primaria">Primaria</option><option value="Secundaria">Secundaria</option>
              </select>
            </div>
            ...
            <div class="form-group" style="grid-column: span 2;"><label class="form-label">Correo Institucional / Personal</label><input type="email" v-model="form.correo" class="input-field" required></div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-ghost" @click="$emit('close')" type="button">Descartar Cambios</button>
...
onMounted(() => {
  if (isEdit) {
    ...
    form.value = {
      matricula: s.matricula, nombreCompleto: s.nombreCompleto, apellidoPaterno: s.apellidoPaterno,
      apellidoMaterno: s.apellidoMaterno, nombres: s.nombres, birth: s.birth ? s.birth.split('T')[0] : '',
      genero: s.genero || '1', plantel: s.plantel || 'PT', nivel: s.nivel, grado: s.grado, grupo: s.grupo,
...
const submit = async () => {
  ...
  try {
    ...
    alert('Falla crítica al interactuar con el backend de expedientes')
  } finally {
    loading.value = false
  }
}

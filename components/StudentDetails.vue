      ...
      () =>
        $fetch("/api/reminders/send", {
          ...
          body: {
            ...
            asunto: "Recordatorio de pago - {{nombre_alumno}}",
            nombreAlumno: props.student.nombreCompleto,
            tutor: props.student.padre,
            matricula: props.student.matricula,
            ciclo: normalizeCicloKey(state.value.ciclo),
            deuda: total,
            plantel: props.student.plantel,
            desglose: validDebts.value.map((debt) => ({
              conceptoNombre: debt.conceptoNombre,
              mesLabel: debt.mesLabel || debt.mes,
              saldo: debt.saldo,
            })),
          },
        }),

...
const toExportValue = (value: unknown): any => {
...
const EXTERNAL_EXPORT_EXCLUDED_KEYS = new Set([
  'completenessTiers',
  'raw',
  'rawPhoto',
  'centralMatriculaRaw',
...
const buildDataColumns = (rows: any[], options: { includeSensitive?: boolean } = {}) => {
  ...
  rows.forEach((row) => {
    Object.keys(row || {}).forEach((key) => {
      if (!knownKeys.has(key) && !EXTERNAL_EXPORT_EXCLUDED_KEYS.has(key)) extraKeySet.add(key)
    })
  })
  const extraKeys = Array.from(extraKeySet)
    .filter((key) => options.includeSensitive !== false || isExternalExportSafeKey(key))
    .sort((a, b) => a.localeCompare(b, 'es'))

  const columns = options.includeSensitive === false
    ? knownColumns.filter(([key]) => isExternalExportSafeKey(key))
    : knownColumns

  return [
    ...columns.map(([key, label, width]) => ({ key, label, width })),
...
export const buildControlEscolarExportWorkbook = async (
  ...
  options: { includeSensitive?: boolean; filenamePrefix?: string; titlePrefix?: string } = {}
    ...
    const workbook = createXlsxWorkbook([
      {
        name: 'Resumen',
        title: `${String(options.titlePrefix || 'Control Escolar')} · Resumen por grado`,
        subtitle,
        columns: [
  ...
  const prefix = String(options.filenamePrefix || 'control-escolar').trim() || 'control-escolar'

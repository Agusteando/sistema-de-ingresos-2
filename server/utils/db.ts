    ...
    const schemaPromise = (async () => {
      ...
      await runSafeQuery(`
        ...
        VALUES (
          ...
          '<div style="font-family: Inter, Arial, sans-serif; color: #1f2937; max-width: 640px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;"><div style="background:#0f766e;color:#fff;padding:18px 24px;"><h2 style="margin:0;font-size:18px;">Recordatorio de Colegiatura</h2></div><div style="padding:24px;"><p style="margin-top:0;">Estimado(a) <strong>{{tutor}}</strong>,</p><p>Le recordamos que la colegiatura del alumno <strong>{{nombre_alumno}}</strong> (Matrícula: <strong>{{matricula}}</strong>) correspondiente al periodo <strong>{{mes}}/{{ciclo}}</strong> presenta saldo pendiente.</p><p>Si ya realizó el pago y se encuentra en proceso de conciliación, por favor haga caso omiso a este mensaje.</p><p>Quedamos atentos para apoyarle en cualquier duda.</p><p style="margin-bottom:0;">Atentamente,<br><strong>Administración y Cobranza</strong></p></div></div>',
          'system'
        )

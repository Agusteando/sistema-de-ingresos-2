...
const isColegiaturaConcept = (conceptoNombre: unknown) => normalize(conceptoNombre).includes('colegiatura')
...
export const getDeudoresColeg = async ({ ciclo, plantel, userEmail }: { ciclo: string, plantel?: string, userEmail?: string }) => {
  const documentos = await query<any[]>(`
    SELECT d.documento, d.matricula, d.costo, d.meses, d.plazo, d.beca, d.conceptoNombre,
  ...
  const documentosColegiatura = documentos.filter(doc => isColegiaturaConcept(doc.conceptoNombre))
  if (!documentosColegiatura.length) return []

  const docIds = documentosColegiatura.map(doc => Number(doc.documento))
  const matriculas = [...new Set(documentosColegiatura.map(doc => String(doc.matricula)))]
  ...
  pagosRows.forEach((p) => {
    const key = `${p.matricula}-${p.documento}-${p.mes}`
  ...
  documentosColegiatura.forEach((doc) => {
    ...
    for (let mes = 1; mes <= plazos; mes++) {
      ...
      const pagos = (pagosByKey.get(`${doc.matricula}-${doc.documento}-${mes}`) || [])
      ...
      const excepcionMes = (excepcionByKey.get(`${doc.matricula}-${mes}`) || []).find((e) => {
        if (!e.fecha_limite_especial) return false
        const limite = new Date(e.fecha_limite_especial)
        return limite >= now
      })

      const key = `${doc.matricula}-${mes}`
      ...
      existing.ultimoMovimiento = evts[0]?.fecha || null
  ...
  return [...bucket.values()].map((row) => {
    // orden determinístico: saldo -> beca100 -> conciliacion -> excepción -> día
    const saldoPendiente = Number(row.saldoColegiatura || 0)
    const noEsDeudorPorSaldo = saldoPendiente <= 0
    const noEsDeudorPorBeca = !noEsDeudorPorSaldo && row.hasBeca100
    const noEsDeudorPorConciliacion = !noEsDeudorPorSaldo && !noEsDeudorPorBeca && row.pagoPendienteConciliacion
    const noEsDeudorPorExcepcion = !noEsDeudorPorSaldo && !noEsDeudorPorBeca && !noEsDeudorPorConciliacion && row.fechaLimiteEspecialVigente

    const isDeudor = !(noEsDeudorPorSaldo || noEsDeudorPorBeca || noEsDeudorPorConciliacion || noEsDeudorPorExcepcion) && day >= 13
  ...
  }).sort((a, b) => b.saldoColegiatura - a.saldoColegiatura || a.nombreCompleto.localeCompare(b.nombreCompleto, 'es'))
}
